...
fn partial_evaluation<T: PrimeField>(polynomial: Vec<T>, evaluation_points: Vec<T>, evaluation_variable_index: usize) -> Vec<T> {
        ...
        while i < polynomial.len() && j < evaluation_points.len() {
            result.push(polynomial[i] * (langrange_basis_zero) + polynomial[i + jump] * (langrange_basis_one));
            if (i + 1) % jump != 0 {
                i = i + 1
            } else {
                i = i + jump;
...
mod tests{
    ...
    fn partial_evaluation_test(){
        let partial_evaluated_result = partial_evaluation(vec![Fq::from(0), Fq::from(2), Fq::from(0), Fq::from(5)], vec![Fq::from(5), Fq::from(0)], 0);
